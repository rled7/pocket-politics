...
export const KEYS = {
  ...
  anthropic: env("ANTHROPIC_API_KEY"),          // AI: Anthropic-native (sk-ant-…)
  // OpenAI-compatible providers (OpenRouter, OpenAI, local Ollama) — one code path, base URL configurable.
  openai: env("OPENAI_API_KEY"),                // sk-or-… (OpenRouter) / sk-… (OpenAI) / any
  openaiBase: env("OPENAI_BASE_URL"),           // e.g. https://openrouter.ai/api/v1 ; default api.openai.com
  llmModel: env("TRANSLATE_MODEL"),             // override model per provider
} as const;
...
const REGISTRY: Integration[] = [
  ...
  { id: "openai",     label: "OpenAI-compatible AI (OpenRouter/OpenAI)", signup: "https://openrouter.ai/keys",          value: KEYS.openai },
];
